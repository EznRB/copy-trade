...
| F1 | Data ingestion **(em andamento)** | BlockchainDataProvider (Helius WSS + RPC), normalização, dedup, storage — código pronto e testado; **pendente: rodar contra mainnet real com Postgres local** | Eventos reais observados e persistidos; **zero trades**; dedup testado (duplicate/reconnect/out-of-order) |
